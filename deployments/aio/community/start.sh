...
print_header(){
    clear
    echo "------------------------------------------------"
    echo "Orchestrix Community (All-In-One)"
    echo "------------------------------------------------"
    echo ""
...
update_env_value(){
    ...
    if [ ! -f "orchestrix.env" ]; then
        echo "orchestrix.env file not found"
        exit 1
    fi

    # check if the key exists and add it if it doesn't
    if ! grep -q "^$key=.*" orchestrix.env; then
        echo "${key}=${value}" >> orchestrix.env
        return 0
    fi

    # if key and value are not empty, update the value
    if [ -n "$key" ] && [ -n "$value" ]; then
        sed -i "s|^$key=.*|$key=$value|" orchestrix.env
        return 0
    fi
...
update_env_file(){
    ...
    # is already stored in orchestrix.env (survives restarts), or generate a fresh random value on
    # first boot. Never write a publicly-known or placeholder value into orchestrix.env.
    ...
    if [ -z "$SECRET_KEY" ] || [ "$SECRET_KEY" = "$_insecure_sk" ] || [ "$SECRET_KEY" = "$_placeholder_sk" ]; then
        local _stored_sk
        _stored_sk=$(grep "^SECRET_KEY=" orchestrix.env 2>/dev/null | cut -d'=' -f2-)
    ...
    if [ -z "$LIVE_SERVER_SECRET_KEY" ] || [ "$LIVE_SERVER_SECRET_KEY" = "$_insecure_lssk" ] || [ "$LIVE_SERVER_SECRET_KEY" = "$_placeholder_lssk" ]; then
        local _stored_lssk
        _stored_lssk=$(grep "^LIVE_SERVER_SECRET_KEY=" orchestrix.env 2>/dev/null | cut -d'=' -f2-)
...
main(){
    ...
    # load orchestrix.env as exported variables
    export $(grep -v '^#' orchestrix.env | xargs)
