...
DOCKER_FILE_PATH=$ORCHESTRIX_INSTALL_DIR/docker-compose.yml
...
function getStackName() {
    read -p "Enter stack name [orchestrix]: " input_stack_name
    if [ -z "$input_stack_name" ]; then
        input_stack_name="orchestrix"
    fi
...
function syncEnvFile(){
    ...
    if [ -f "$ORCHESTRIX_INSTALL_DIR/orchestrix.env.bak" ]; then        
        ...
        value=$(getEnvValue "STACK_NAME" "$ORCHESTRIX_INSTALL_DIR/orchestrix.env.bak")
        if [ -n "$value" ]; then
            updateEnvFile "STACK_NAME" "$value" "$DOCKER_ENV_PATH"
        fi
    fi
    echo "Environment variables synced successfully" >&2
    rm -f $ORCHESTRIX_INSTALL_DIR/orchestrix.env.bak
}
...
function download() {
    ...
    TS=$(date +%s)
    if [ -f "$ORCHESTRIX_INSTALL_DIR/docker-compose.yml" ]; then
        mv $ORCHESTRIX_INSTALL_DIR/docker-compose.yml $ORCHESTRIX_INSTALL_DIR/archive/$TS.docker-compose.yml
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $ORCHESTRIX_INSTALL_DIR/docker-compose.yml
    else
        ...
        if [ "$STATUS" -eq 200 ]; then
            ...
            mv $ORCHESTRIX_INSTALL_DIR/archive/$TS.docker-compose.yml $ORCHESTRIX_INSTALL_DIR/docker-compose.yml
            exit 1
        fi
    fi
