...
SCRIPT_DIR=$PWD
SERVICE_FOLDER=orchestrix-app
ORCHESTRIX_INSTALL_DIR=$PWD/$SERVICE_FOLDER
export APP_RELEASE=stable
export DOCKERHUB_USER=ezeslucky
export PULL_POLICY=${PULL_POLICY:-if_not_present}
export GH_REPO=ezeslucky/orchestrix
...
mkdir -p $ORCHESTRIX_INSTALL_DIR/archive
DOCKER_FILE_PATH=$ORCHESTRIX_INSTALL_DIR/docker-compose.yaml
DOCKER_ENV_PATH=$ORCHESTRIX_INSTALL_DIR/orchestrix.env
...
function initialize(){
    ...
    local IMAGE_NAME=ezeslucky/orchestrix-proxy
    ...
    if [ $? -eq 0 ]; then
        echo "orchestrix supports ${CPU_ARCH}" >&2
        echo "available"
        return 0
    else
        echo "" >&2
...
function syncEnvFile(){
    ...
    if [ -f "$ORCHESTRIX_INSTALL_DIR/orchestrix.env.bak" ]; then
        updateCustomVariables
        
        # READ keys of orchestrix.env and update the values from orchestrix.env.bak
        ...
        do
            ...
            value=$(getEnvValue "$key" "$ORCHESTRIX_INSTALL_DIR/orchestrix.env.bak")
            if [ -n "$value" ]; then
...
function buildYourOwnImage(){
    ...
    export DOCKERHUB_USER="myorchestrix"
    export APP_RELEASE="local"
    export PULL_POLICY="never"
    CUSTOM_BUILD="true"

    # checkout the code to ~/tmp/orchestrix folder and build the images
    local ORCHESTRIX_TEMP_CODE_DIR=~/tmp/orchestrix
    rm -rf $ORCHESTRIX_TEMP_CODE_DIR
    mkdir -p $ORCHESTRIX_TEMP_CODE_DIR
    REPO=https://github.com/$GH_REPO.git
    git clone "$REPO" "$ORCHESTRIX_TEMP_CODE_DIR"  --branch "$BRANCH" --single-branch --depth 1

    cp "$ORCHESTRIX_TEMP_CODE_DIR/deployments/cli/community/build.yml" "$ORCHESTRIX_TEMP_CODE_DIR/build.yml"

    cd "$ORCHESTRIX_TEMP_CODE_DIR" || exit
...
function install() {
    echo "Begin Installing orchestrix"
    echo ""
...
function download() {
    ...
    cd $SCRIPT_DIR
    TS=$(date +%s)
    if [ -f "$ORCHESTRIX_INSTALL_DIR/docker-compose.yaml" ]
    then
        mv $ORCHESTRIX_INSTALL_DIR/docker-compose.yaml $ORCHESTRIX_INSTALL_DIR/archive/$TS.docker-compose.yaml
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $ORCHESTRIX_INSTALL_DIR/docker-compose.yaml
    else
        ...
        if [ "$STATUS" -eq 200 ]; then
            ...
            mv $ORCHESTRIX_INSTALL_DIR/archive/$TS.docker-compose.yaml $ORCHESTRIX_INSTALL_DIR/docker-compose.yaml
            exit 1
        fi
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $ORCHESTRIX_INSTALL_DIR/variables-upgrade.env
    else
        # Fallback to download from the raw github url
    ...
    then
        cp "$DOCKER_ENV_PATH" "$ORCHESTRIX_INSTALL_DIR/archive/$TS.env"
        cp "$DOCKER_ENV_PATH" "$ORCHESTRIX_INSTALL_DIR/orchestrix.env.bak"
    fi

    mv $ORCHESTRIX_INSTALL_DIR/variables-upgrade.env $DOCKER_ENV_PATH
    ...
    echo ""
    echo "Most recent version of orchestrix is now available for you to use"
    echo ""
    echo "In case of 'Upgrade', please check the 'orchestrix.env 'file for any new variables and update them accordingly"
    echo ""
}
function startServices() {
    ...
    if [ -n "$migrator_container_id" ]; then
        ...
        if [ $migrator_exit_code -ne 0 ]; then
            echo "orchestrix Server failed to start ❌"
            # stopServices
            echo
    ...
    echo "   orchestrix Server started successfully ✅"
    echo ""
...
function upgrade() {
    ...
    echo "Upgrading orchestrix to the latest release..."
    echo ""
...
function viewLogs(){
    ...
    then
        ...
        then
            ...
            case $DOCKER_SERVICE_NAME in
                ...
                8) viewSpecificLogs "orchestrix-redis";;
                9) viewSpecificLogs "orchestrix-db";;
                10) viewSpecificLogs "orchestrix-minio";;
                11) viewSpecificLogs "orchestrix-mq";;
                0) askForAction;;
                *) echo "INVALID SERVICE NAME SUPPLIED";;
            esac
        fi
    ...
    then
        ...
        case $ARG_SERVICE_NAME in
            ...
            redis) viewSpecificLogs "orchestrix-redis";;
            postgres) viewSpecificLogs "orchestrix-db";;
            minio) viewSpecificLogs "orchestrix-minio";;
            rabbitmq) viewSpecificLogs "orchestrix-mq";;
            *) echo "INVALID SERVICE NAME SUPPLIED";;
        esac
    else
        echo "INVALID SERVICE NAME SUPPLIED"
...
function backupData() {
    ...
    local BACKUP_FOLDER=$ORCHESTRIX_INSTALL_DIR/backup/$datetime
    ...
    backup_container_dir "$BACKUP_FOLDER" "orchestrix-db" "/var/lib/postgresql/data" "pgdata" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "orchestrix-minio" "/export" "uploads" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "orchestrix-mq" "/var/lib/rabbitmq" "rabbitmq_data" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "orchestrix-redis" "/data" "redisdata" || exit 1
