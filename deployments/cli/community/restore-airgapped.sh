...
function restoreData() {
    ...
    AIRGAPPED_INSTALL_PATH="$HOME/orchestrixairgapped"
    ...
    if command -v jq &> /dev/null; then
        dockerServiceStatus=$($COMPOSE_CMD ls --filter name=orchestrix-airgapped --format=json | jq -r .[0].Status)
    else
        dockerServiceStatus=$($COMPOSE_CMD ls --filter name=orchestrix-airgapped | grep -o "running" | head -n 1)
    fi

    if [[ $dockerServiceStatus == "running" ]]; then
        echo "orchestrix Airgapped is running. Please STOP the orchestrix Airgapped before restoring data."
        exit 1
    fi
